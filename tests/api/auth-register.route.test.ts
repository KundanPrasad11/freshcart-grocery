...
const register = (body: unknown, ip = "198.51.100.8") =>
  POST(
...
describe("POST /api/auth/register", () => {
  ...
  it("rate limits repeated registrations from one client", async () => {
    for (let index = 0; index < 5; index += 1) {
      expect(
        (
          await register(
            { name: "Jamie Rivera", email: `jamie-${index}@example.test`, password: "password123" },
            "203.0.113.5"
          )
        ).status
      ).toBe(201);
    ...
    const response = await register(
      { name: "Jamie Rivera", email: "jamie-6@example.test", password: "password123" },
      "203.0.113.5"
    );
  ...
  it("rejects non-JSON and oversized registration bodies", async () => {
    expect(
      (
        await POST(
          new Request("http://test/api/auth/register", {
